<div class="slider-area">
    <div class="hero-slider owl-carousel">
        <div class="single-slider" style="background-image:url(https://res.cloudinary.com/ikutaja/image/upload/v1546490553/img/slider/1.jpg)">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="hero-slider-content">
                            <h1>Bantuanmu membuat hidup kita lebih baik</h1>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="single-slider" style="background-image:url(https://res.cloudinary.com/ikutaja/image/upload/v1546490270/img/slider/2.jpg)">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="hero-slider-content">
                            <h1>Ajaranmu akan membuat dunia lebih baik</h1>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<div class="service-area">
    <div class="container">
        <div class="row">
            <div class="col-lg-7">
                <div class="row">
                    <div class="col-12">
                        <div class="service-section-title mb-40">
                            <h2>How It's Work!</h2>
                            <h3>Pemimpin dalam penggalangan dana online</h3>
                            <p>Ikutaja dipercaya di seluruh dunia untuk platform penggalangan dana yang sederhana dan handal.</p>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-6">
                        <div class="single-service mb-30">
                            <h2 class="service-title">
<i class="fa fa-cogs" aria-hidden="true"></i>
Pengaturan sederhana
</h2>
                            <p class="pl-30">Anda dapat mempersonalisasi dan membagikan Rate Ikutaja Anda, hanya dalam beberapa menit.</p>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="single-service mb-30">
                            <h2 class="service-title">
<i class="fa fa-users" aria-hidden="true"></i>
Jangkauan sosial</h2>
                            <p class="pl-30">Manfaatkan kekuatan media sosial untuk menyebarkan kisah Anda dan dapatkan lebih banyak dukungan.</p>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="single-service mb-30">
                            <h2 class="service-title">
<i class="fa fa-building-o" aria-hidden="true"></i>
Konstribusi Perusahaan</h2>
                            <p class="pl-30">Perusahaan-perusahaan dermawan dan karyawan mendukung proyek-proyek kami, dan membantu komunitas lokal berkembang. </p>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="single-service mb-30">
                            <h2 class="service-title">
<i class="fa fa-commenting-o" aria-hidden="true"></i>
Live Chat 24/7</h2>
                            <p class="pl-30">Agen Agen kami yang terbaik akan menjawab pertanyaan Anda, siang atau malam.</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="service-image">
        <img draggable="false" src="https://res.cloudinary.com/ikutaja/image/upload/v1546490688/img/service/1.png" alt="">
    </div>
</div>
<div id="event"></div>
<div class="event-area pt-100">
    <div class="container">
        <div class="row align-items-center">
            <div class="pt-50 col-lg-6 col-12">
                <div class="event-slider">
                    <div class="single-event">
                        <div class="event-img img-full">
                            <img draggable="false" src="https://res.cloudinary.com/ikutaja/image/upload/v1546101532/img/event/1.jpg" alt="">
                        </div>
                        <div class="event-content">
                            <h3>Medical Camp for Poor Children</h3>
                            <p>Central Hall, California. 25 Oct, 2018 at 10 am</p>
                        </div>
                    </div>
                    <div class="single-event">
                        <div class="event-img img-full">
                            <img draggable="false" src="https://res.cloudinary.com/ikutaja/image/upload/v1546101532/img/event/2.jpg" alt="">
                        </div>
                        <div class="event-content">
                            <h3>Pure Drinking Water for helpless people</h3>
                            <p>Midland Cricket Ground. 25 Oct, 2018 at 10 am</p>
                        </div>
                    </div>
                    <div class="single-event">
                        <div class="event-img img-full">
                            <img draggable="false" src="https://res.cloudinary.com/ikutaja/image/upload/v1546101532/img/event/3.jpg" alt="">
                        </div>
                        <div class="event-content">
                            <h3>Medical Camp for Poor Children</h3>
                            <p>Central Hall, California. 25 Oct, 2018 at 10 am</p>
                        </div>
                    </div>
                    <div class="single-event">
                        <div class="event-img img-full">
                            <img draggable="false" src="https://res.cloudinary.com/ikutaja/image/upload/v1546101532/img/event/4.jpg" alt="">
                        </div>
                        <div class="event-content">
                            <h3>Pure Drinking Water for helpless people</h3>
                            <p>Midland Cricket Ground. 25 Oct, 2018 at 10 am</p>
                        </div>
                    </div>
                    <div class="single-event">
                        <div class="event-img img-full">
                            <img draggable="false" src="https://res.cloudinary.com/ikutaja/image/upload/v1546101532/img/event/5.jpg" alt="">
                        </div>
                        <div class="event-content">
                            <h3>Medical Camp for Poor Children</h3>
                            <p>Central Hall, California. 25 Oct, 2018 at 10 am</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-lg-6 col-12 pl-60">
                <div class="section-title text-left">
                    <h2>Event Mendatang</h2>
                </div>
                <div class="event-description">
                    <h4>Update informasi acara terbaru dan yang akan datang dengan jelas ...</h4>
                    <p>Mengejar kesenangan secara rasional kemudian menghadapi konsekuensi yang menyakitkan. Meminimalisir orang orang yang merasa sakit dan kurang.</p>
                    <p>Kami percaya akan hadir peradaban dunia yang lebih baik. Semua masalah kemanusiaan dapat diselesaikan dengan membangun kedermawanan dan kerelawanan global.</p>
                    <a class="default-btn" href="<?php echo base_url('events'); ?>">SELEBIHNYA</a>
                </div>
            </div>
        </div>
    </div>
</div>
<div class="fun-factor-area fun-bg mt-100">
    <div class="container">
        <div class="row justify-content-between content-between">
            <div class="col">
                <div class="single-funfactor mb-30 text-center">
                    <div class="fun-facttor-number">
                        <h2><span class="counter">1280</span></h2>
                    </div>
                    <h4 class="counter-title">Proyek Sukses</h4>
                </div>
            </div>
            <div class="col">
                <div class="single-funfactor mb-30 text-center">
                    <div class="fun-facttor-number">
                        <h2><span class="counter">1052</span></h2>
                    </div>
                    <h4 class="counter-title">Total Sponsor</h4>
                </div>
            </div>
            <div class="col">
                <div class="single-funfactor mb-30 text-center">
                    <div class="fun-facttor-number">
                        <h2>$<span class="counter">280</span>m</h2>
                    </div>
                    <h4 class="counter-title">Total Terdonasi</h4>
                </div>
            </div>
            <div class="col">
                <div class="single-funfactor mb-30 text-center">
                    <div class="fun-facttor-number">
                        <h2><span class="counter">3298</span></h2>
                    </div>
                    <h4 class="counter-title">Total Sukarelawan</h4>
                </div>
            </div>
        </div>
    </div>
</div>
<div class="testimonial-area pt-100 pb-100">
    <div class="container">
        <div class="row testimonial-active">
            <div class="col-6">
                <div class="single-testimonial">
                    <div class="testimonial-content">
                        <p>Secara umum, IkutAja.id sudah baik dan pelaporan dari donasi cukup jelas. Senang bisa membantu teman-teman yang membutuhkan lewat IkutAja.id. Terimakasih IkutAja.id</p>
                    </div>
                    <div class="testimonial-author-info">
                        <div class="team-author-deg">
                            <h3>Achmad Ridwan</h3>
                            <p>CEO Galang Dana</p>
                        </div>
                        <div class="author-img">
                            <img draggable="false" src="https://res.cloudinary.com/ikutaja/image/upload/v1546102714/img/testimoni/1.jpg" alt="">
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-6">
                <div class="single-testimonial">
                    <div class="testimonial-content">
                        <p>IkutAja.id adalah tempat yang membuat saya nyaman untuk penyaluran donasi karena pengalokasian donasinya yang transparan dan ada laporan perkembangan donasi pasien.</p>
                    </div>
                    <div class="testimonial-author-info">
                        <div class="team-author-deg">
                            <h3>Siska Dewi</h3>
                            <p>CEO Indo Volunteer</p>
                        </div>
                        <div class="author-img">
                            <img draggable="false" src="https://res.cloudinary.com/ikutaja/image/upload/v1546102714/img/testimoni/2.jpg" alt="">
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-6">
                <div class="single-testimonial">
                    <div class="testimonial-content">
                        <p>IkutAja.id telah memberi saya kepercayaan diri dan keyakinan yang kuat bahwa uang yang saya sumbangkan akan diberikan kepada orang yang tepat dan untuk tindakan yang benar.</p>
                    </div>
                    <div class="testimonial-author-info">
                        <div class="team-author-deg">
                            <h3>Nita Aisyah</h3>
                            <p>CEO Simpan Berikan</p>
                        </div>
                        <div class="author-img">
                            <img draggable="false" src="https://res.cloudinary.com/ikutaja/image/upload/v1546102714/img/testimoni/3.jpg" alt="">
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-6">
                <div class="single-testimonial">
                    <div class="testimonial-content">
                        <p>IkutAja.id adalah sebuah platform yang bisa menghubungkan kita dengan kekuatan publik. Sangat cepat dan transparan. Saya sudah pakai dua kali untuk galang dana bencana alam.
                        </p>
                    </div>
                    <div class="testimonial-author-info">
                        <div class="team-author-deg">
                            <h3>Nadiem Mustofa</h3>
                            <p>CEO Donasi Kita</p>
                        </div>
                        <div class="author-img">
                            <img draggable="false" src="https://res.cloudinary.com/ikutaja/image/upload/v1546102714/img/testimoni/4.jpg" alt="">
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<div class="project-area">
    <div class="container-fluid p-0">
        <div class="row no-gutters project-active owl-carousel">
            <div class="col-12">
                <div class="single-project img-full">
                    <a href="https://www.cimbclicks.co.id/ib-cimbniaga/Login.html"><img draggable="false" src="https://res.cloudinary.com/ikutaja/image/upload/v1546102471/img/sponsor/1.png" alt=""></a>
                </div>
            </div>
            <div class="col-12">
                <div class="single-project img-full">
                    <a href="https://www.go-jek.com/"><img draggable="false" src="https://res.cloudinary.com/ikutaja/image/upload/v1546102471/img/sponsor/2.png" alt=""></a>
                </div>
            </div>
            <div class="col-12">
                <div class="single-project img-full">
                    <a href="http://www.amikom.ac.id/"><img draggable="false" src="https://res.cloudinary.com/ikutaja/image/upload/v1546491250/img/sponsor/3.png" alt=""></a>
                </div>
            </div>
            <div class="col-12">
                <div class="single-project img-full">
                    <a href="https://ib.bri.co.id/ib-bri/"><img draggable="false" src="https://res.cloudinary.com/ikutaja/image/upload/v1546102471/img/sponsor/4.png" alt=""></a>
                </div>
            </div>
            <div class="col-12">
                <div class="single-project img-full">
                    <a href="https://www.grab.com/id/"><img draggable="false" src="https://res.cloudinary.com/ikutaja/image/upload/v1546102471/img/sponsor/5.png" alt=""></a>
                </div>
            </div>
        </div>
    </div>
</div>